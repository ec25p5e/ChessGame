...
/**
 * Questa classe serve per creare una mossa d'arrocco verso il lato del re
 */
public class KingSideCastleMove extends CastleMove {
    public KingSideCastleMove(final VirtualBoard board, final Piece pieceToMove, final int destinationCoordinate, final Rook castleRook, final int castleRookStart, final int castleRookDestination) {
        super(board, pieceToMove, destinationCoordinate, castleRook, castleRookStart, castleRookDestination);
    }

    /**
     * Questo metodo serve per confrontare un oggetto a un oggetto della classe {@link KingSideCastleMove}
     * @param other oggetto da confrontare
     * @return valore booleano di valore "TRUE" se l'oggetto è un'istanza di {@link KingSideCastleMove}
